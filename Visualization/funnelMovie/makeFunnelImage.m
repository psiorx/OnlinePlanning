function [im_funnel,figNum] = makeFunnelImage(funnel,funnelPos,camPosition,camOrientation,camUp,cameraParameters)
...
figNum = 99;
figure(figNum);
set(figNum,'Position',[100 100 im_sz],'Visible','Off');
% figNum = figure('Position',[100 100 im_sz],'Visible','Off');
options = struct();
options.inclusion = 'projection';
options.ts = funnel.ts;
options.x0 = funnel.xtraj;
options.plotdims = [1 2 3];
options.num_samples = 1000;

plotShiftedFunnel(funnel.V,funnelPos,options);
