/* Produced by CVXGEN, 2013-07-30 13:25:10 -0400.  */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 3) && (mxGetN(xm) == 3))) {
      printf("S must be size (3,3), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 3))) {
      printf("s1 must be size (1,3), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "s2");
  if (xm == NULL) {
    printf("could not find params.s2.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("s2 must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter s2 must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter s2 must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter s2 must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.s2;
