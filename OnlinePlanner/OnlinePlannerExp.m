classdef OnlinePlannerExp < DrakeSystem
    ...
    methods
        ...
        % construction
        function obj = OnlinePlannerExp(funnelLibrary,forestAhead,currentFunnel,cyclicIdx,t_replan,t_start,x_start,num_inputs,x_trigger,t_end_exp,robot,u_trim,options)
            ...
            % Replan a few times because Matlab is always slow on first call
            options_dummy = struct(); % dummy options to make sure we call shifting code.
            options_dummy.shift_method = 'qcqp';
            options_dummy.penetration_thresh = -1000.0; 
            options_dummy.failsafe_penetration = -1000.0; 
            for k = 1:5
                dummyFunnel = obj.replanFunnels(t_start,x_start,options_dummy);
            end
            
            obj.currentFunnel = SharedDataHandleMutable(obj.currentFunnel);
            
            obj.currentFunnel.setData(1);
            
            % Call plant dynamics a few times
            for k = 1:5
        ...
        function y = output(obj,t,~,x)
            ...
            % Check if nextFunnel = 0 (failsafe)
            if (obj.currentFunnel.getData() == 0)
                y = [150;512;512;512];
                return;
            end
            ...
            elseif t_current > obj.t_replan
                ...
                V_val = x_current'*Snow*x_current + s1now'*x_current + s2now;
                insideCurrent = (V_val < 1);
                ...
                if insideCurrent
                    ...
                    if collisionFree
                        ...
                        replan = false; % FIXME: false for experiments
                    end
                    
                else % If not inside current funnel anymore, replan
                    replan = false; % FIXME: false for experiments
                    % disp(['Out of current funnel at t = ' num2str(t_current) '. Need to replan.']);
                    % V_val
                end
            end
            ...
            if replan
                ...
                nextFunnel = obj.replanFunnels(tpred,xpred,obj.options); % should be t,x if there is no delay
        ...
        function nextFunnel = replanFunnels(obj,t,x,options)
           ...
           tic
           [nextFunnel,x_execute_next,collFree,min_dist] = replanFunnels_mex(x,forest,obj.funnelLibraryReduced,options); 
           toc
           
           x_execute_next
           
           % Set start time for new funnel
           obj.t_start.setData(t);
            
           % Set start state for new funnel
           % obj.x_start.setData(x);
           obj.x_start.setData(x_execute_next);
